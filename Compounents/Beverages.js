...
const Beverages = () => {
    ...
    const fetchedData = [];
    const getBaverages = async () => {

        try {
    
          const querySnapshot = query(collection(db, "items"), where("category", "==", "Beverages"));
          ...
          setBeverages(Object.values(fetchedData))
          console.log('dessss', Beverages);
        } catch (error) {
          console.log('failed to get Beverages');
        }
